---
title: "Web3 and Distributed Ledger Technology - Blockchain Basics (Part 3)"
read_time: false
...
It's in Cologne, but I think it is very interesting.
It's organized by the Aachen Blockchain Club, by the Crypto-Kölsch.
Crypto-Kölsch is just a few people who organize events from time to time.
The name is coming from the Kertl and Kerch, which is the beer they have been drinking.
They organize meetings from time to time.
Last time we had a meeting they came to us and we had a meeting on the **Polymarket**.
It was about the prediction markets.
We had someone from [Dune](https://docs.dune.com/) who was talking about Polymarket.
This time they are being sponsored by BASE, so **Coinbase**.
I think we love things for free.
You will get Glühwein, Christmas cookies, NFT, Christmas postcard competition.
If you are near Cologne, it's probably nice to get there.
It's in the Startplatz.
Startplatz is a location in Cologne, which is close to the big cinema in Cologne.
There is a large cinema in Cologne.
It's reachable by train.
Even the train from Aachen to Cologne stops there.
You need to exit Ehrenfels, and then you are almost there.
Just look for it. It's Crypto-Kerch, Christmas meetup.

## org: exam date

The other thing was the poll.
Someone reminded me.
I'm not sure if I should show you all the names.
We have nine now, father nine.
I have just looked it up and I have no idea if we exclude very many people.
There is not much over that.
Did you all vote already?
What shall we do?
I'll take the mic because I already have to answer it.
It seems like the last spread.
The other one would be the eighth.
The eighth seems to be also very obvious.
It's the eighth problematic.
Very problematic.
Any big, big objections.
This one here, the twenty-first.
Nobody is really objecting against it.
Let me see.
Ah, I can just select the people.
For the moment it's the twenty-first.
Good.
Let me see.
I'll send the short notice about this.
Maybe I can just select it by myself.
Good.
Now we have...
That's nine.
Good.
...
I think the last time we stopped over here.
We said **"not your keys, not your crypto"**.
I think what we wanted to do today was to install a wallet.
For all of you who have not installed a wallet yet,
you can just do it together.
I just installed it here.
If you like you can follow on your smartphone or on your laptop.
The easiest would be to get out of here
and I go to Chrome because I think in Chrome
...
The easiest way to do it is to just look for a wallet.
This is something that is used by a lot of people
which is probably **not the most convenient one**.
But we just selected it for the moment.
We just look for **Metamask, an open source tool**.
We look for Metamask, it's an open source tool from the web store
and just add it.
Just another **extension that we add to our browser** here.
Some time and then this has been added.
Now the thing about Chrome is
you see it always looks like this fox.
I think it's a fox.
Then we just say let's create a new wallet.
We could start with some kind of log in
and then probably retrieve it from the cache that we have there.
But we just use here this **secret recovery phrase**.
Then first we have to provide a **password**.
Last time I hope you remember that I said
**there is no password on the blockchain**
and now it's asking you for a password.
...
I just created with my password manager
a password in your account last picture
because otherwise it would be immediately
and probably just...
Thank you.
So...
Thank you.
...
And...
Say, okay.
So what it's now doing is the following.
Now it's testing whether I really remember this particular secret phrase.
If I don't remember that,
then it wouldn't let me go on further.
That's very important.
So now it's asking me to provide particular words.
So what I will do now is that I, again,
put this take here onto different screens.
So here,
and I need to get my...
And you see, I'm going to get my...
Okay, so here I have now my copy phrases.
And what I would need to do now is that it's asking me,
what's the number five?
Record number seven.
That's the number ten.
Okay.
Okay.
So now you know at least three of my secret words.
And you know the position, but you don't know the rest.
So obviously I would be able to do a lot of money on this wallet.
But it's the point.
...
Sometimes it's just a website that you just have to keep open for some time.
And then you get a few of these play tokens, test tokens, more or less.
So, as you see, we call Sepolia.
Sepolia.
And you see it's not there.
It's somewhere in the token for a case to change.
Yeah.
Okay.
So, let me see.
Networks.
There is no way to edit.
They changed somehow the user.
...
Yeah.
Yeah.
And now you see Sepolia.
Yes.
And you say, okay, Sepolia.
It should be there.
No, no, it should be there.
It's actually there.
So, but just by...
You see if we have Sepolia in my wallet here.
It should be the same as the T-re-m-a-d-r.
Sorry.
It should be the same as the T-re-m-a-d-r.
The same as the T-re-m-a-d-r.
Yeah.
But I don't see it.
It's not being shown here.
Maybe a 5 cent money.
Maybe it's being shown.
That's okay.
Sepolia.
What I'm saying.
...
And he's now I could also take this and copy it for example into the moon and ask you to send me one.
No, just before the exam.
Okay.
Yeah.
So that's the that's the.
And not even sending me something.
Yeah.
...
So always remember, the blockchain that shared global state.
Yeah.
So if you want to achieve.
And for it just a note, which is connected to the working to create your own.
No, this is the only thing you can send the transaction.
But in that case, because it's complicated.
You know, I'm creating a lot of work.
I send the money, but I'm not sure whether it's.
It's not working.
I can send you something on main.
And I can send you something on the main.
And one of the companies.
So normally it's something like.
Yeah.
Maybe you can.
Yeah.
Maybe you can.
...
but,
okay,
it's telling me that it can't be about in 12 seconds,
it should be being performed,
and that from lecture,
which is just the frankly name that I gave it,
to this address here,
and now I send it to him.
...
I was here.
I thought you were not going to go.
You weren't going to go?
I thought it was showing all of the power.
Okay, there we go.
So, you sent me 0.01e,
which is at the moment
three dollars, which was...
So, I think I have to send it back to you.
Otherwise, they'd say,
oh, you've got a...
Right, right.
And you sent a euro amount,
but anything, because if you go in the store
and say, I kind of pay with Ethereum,
they probably won't have Ethereum credit.
Yes, and no, you can't.
Either they accept Ethereum,
or I swap it against another token.
So, this is what you see down here.
Sir, can I type in somewhere?
Swapping Ethereum,
...
Normally, they take euros.
But if you go to another country,
...
Normally, you lose like that.
So you have to be careful,
if they have provided a good exchange rate or not.
So I was playing around,
in particular in order to get some USDC,
so I was also changing some heat,
and it went also from another chain,
and so on.
...
we can call it the lab free lecture token or whatever,
you could also call it euro,
you can give it somehow a euro symbol here.
But it's somehow misleading,
because I could also call it USDC or whatever,
...
so I am still looking for the point
where do I see your address?
Oh yeah, why are there, for example,
Ethereum chains showing with different exchange rates?
...
I think it was in the bottom.
The bottom, should be something here.
I, that's your address, sorry.
Yeah, on what?
Seven, and your address?
So it's the 7815.
7814, yeah, okay.
7818.
Okay, 87, your diesel, yeah.
Okay, so I should send it back to him.
So 781872, yeah, good.
And max.
Okay.
Now it's getting back in.
It was like 10 cents, 12 cents in the cash.
That's a six, yeah.
...
Okay, your transaction is complete.
You send to receive, and it's like that.
That's the receiving value,
and block number, so we see here.
All right, that's a good transaction.
So you see when we go back now,
to lecture,
so I still have zero, one, two, seven.
Okay, so it didn't send all of them.
So maybe, how could this happen?
Interesting, maybe it just calculated transaction fees,
and therefore it calculated how much it can send,
...
you go to a particular box,
and you go to a box office,
and there you exchange your role against plastic tokens.
And that's the currency, yeah?
The blue wine has three tokens,
a raiboukken, hashbrowns,
there's one token, one of these blue things,
and that then frees all the people
...
I mean, the point that this is worth now 7 cents,
is just because someone in the world
is willing to pay me 7 cents for that.
Actually, these are just some bits and bytes,
some are on the computer.
It's the same that someone is willing to pay me 10 euros,
or five euros here for this particular piece of paper.
It's just the point that someone is willing to pay me,
so I take it to that.
Actually, here with my,
this over here, go back to this browser.
No, this one, no.
...
I think we should have provided something
at a exchange or something.
Oh, okay.
...
to another software,
because coinbase or metamask is just a different software,
...
Okay, and what I can do possible to do now
is if I go back to this one,
and I can send money,
you see, we go back to our,
that's it, we try to send this one around,
and our next one too, possible, no.
No, no, no, add in the,
no, no, no, no, no, no, no, no, no, no, no, no, no, no,
this one, lecture two, and we take max.
...
And you can also create the third one.
So if I like, I can also create a third one.
...
This is just protecting the wallet itself on my computer.
And you can have, for example,
like you have the private key in MetaMask,
you can have one password,
and then you have the private key in Coinbase,
you can have a different password for Coinbase,
...
Because you see, I just divided it up
into different accounts over here.
So there is a master, and then there's one thing,
that I use for our example, NFTs and some tests,
you see they're all empty,
because they, for example, just contain some NFTs.
And they were gone.
But then when in Coinbase, I said, create a new account.
...
in each account.
And it's somehow a deterministic way how you do that.
There is no random in there,
because otherwise you couldn't start.
So you see here on this particular account,
I just called it NFT, yeah?
So I don't have any tokens.
...
And also when I created this particular wallet
with this new address here, three something.
Then I created this one here,
and you see that here on lecture one,
...
And what my wallet does actually is that
when it addresses accesses in Fuwa,
it asks in Fuwa, hey, I have this particular user here
with this particular address.
Tell me about the money and the currencies and tokens
that NFTs this particular user owns.
So it's not in the wallet.
It caches some information.
But when I open it up, it's just getting to the server,
getting my amounts, and then it's showing me.
That's just what's a lot to have in there.
Yeah, so I can just carry it to you,
just create your own wallet,
play around a little bit, was it?
...
You will also see that when we go back here to,
this one, there are some NFTs,
that are created.
That's also a very interesting thing.
In MetaMask, for example,
it's very uncomfortable with NFTs.
Now, in order to show the NFTs that you own,
here you would need to tell MetaMask
which smart contract it is,
and what kind of NFT number it is.
So you really have to tell this wallet a lot
about the things on the blockchain.
This coinbase, the big thing is that coinbase
really knows about all the NFTs
and knows how to access them.
And when I go to the NFT down here,
and when I say show me all the NFTs,
you see, I see a lot.
So it's the same thing, it's my NFT address,
but it shows me all the other NFTs also
that we were playing around with.
Here I don't need to tell the system about the smart contract
I'm working with, and which particular NFT number it is,
just shows me everything,
just retrieves everything from the data.
So what they actually do is they keep somehow a database
...
and when one adds it,
only then one can see the NFTs and the values there.
...
then you can just create the other ones.
You see it's very simple, plus, plus, plus, plus, plus,
and then all your accounts are there.
So there's not much effort.
So it's really, you can consider a wallet
somehow as some kind of filter,
a filter on the blockchain.
So it's really something like glasses
that you use to look at the blockchain,
and then you only see things that are owned by you.
And the wallet itself checks this by just checking
if you can produce the appropriate challenge
for the particular address for which this token is stored
on the blockchain.
There's an entry which says that this particular
035 address that this token is owned by 035.
And in order to prove that this is me,
prove this is me, I need to be able to sign
the particular challenge with my secret key
that the blockchain can test,
that only I could do that because I know the secret key,
which is another one.
Okay, so yeah, so much about this.
...
you enter your address and then it's gone.
And then you have stolen me an NFT.
So easy this.
And it's the same as if you would tell me
your secret password for your banking app.
Yeah, so it's actually the same,
but you have to be careful with it.
And also, what do I know now?
I know now two addresses.
I know Ivan's address and I know his address.
...
that was yours then.
So, and this is the address of Ivan
and all the transactions that you did.
...
send it to another account locally,
and then from there sending it to me.
...
Like I think that there are cyclone smart contracts
where a lot of people send money in
...
So what we did is we just invested some money in the middle
but the transactions are outside
with this voucher that we are sending around.
Like email or that could be a secret number or whatever.
And you will use that as a secret number
to get the money out of there.
And this means nobody can track that I gave him money.
And therefore we can do some very,
it's a nice way to do some money laundering
and therefore to native cash is legal.
I don't think so.
...
So you see here is where he addressed this particular method.
So what he did was he did a transfer
to this particular address.
And what he did here is that security by our IT
has to lock down, it's up for five minutes.
And he thought in this ago, he created a contract,
he created it to do this, with this particular account,
...
So we are looking at the smart contract now
and this smart contract.
I can look up the contract itself
and you can see what kind of method it owns to read
...
If you sent me 100 euros and then you call the bank and you tell them, oh, sorry, that was a mistake,
I'm not sure that they will immediately reverse it because maybe that I gave you some value for that.
I sold you something for 100 euros and if the bank would just say, oh, it was a mistake by him,
this would make the whole bank no longer trusted.
It can't be done on the blockchain account.
That's the reason why, you saw, I was double checking, is that really your account?
...
Every byte costs money on the blockchain.
And then if you really make a stake to make that a data type,
that just can store 256 values,
suddenly you have a big trouble in the smart contract.
So these are really things that you have to observe when you develop smart contracts.
Much more than with any other kind of program,
because you really use very strange data types.

## slide: "Sustainability - Energy consumption"

see 1:06:00

Okay, so let's go back to these slides.
That was something... I didn't show that, right? Good.

So that is something that now comes across, the big discussion, sustainability of blockchain,
in particular, proof of work.
You know, it takes a lot of energy to compute the proof of work,

## slide: "Energy Consumption by Country"

and sometimes you see these kind of charts, you see countries, it's old, I think it's old.
But it's somewhere going in this area between 20 and 29 and back and forth.
And you see **Bitcoin has the same energy consumption as Thailand or Poland**.
So that is something where people say, wow, that's too much.
Why should we do that?
And that's a lot of critique on that one.

I'm not sure.
I would like to see now a chart comparing this with ChatGPT,
which is similar, but that was the big issue.

And **four to five years ago**, everybody was talking about that.
Why is this so **energy-intensive**? we **need to find another solution**.

## slide: "Validation of Transactions in a Community - Consensus building"

But then the other **solution** is really then to **think about other consensus-building methods**.

- So this is **proof of work**.
  - So it's solving a crypto-puzzle, and
  - it requires energy, computing, performance and time.
- What we could do is, we can run a blockchain, everybody of us is running a node, and we just have **one** very **explicit validation node**.
  - BigchainDB, in the areas in the **old blockchains, in particular, did this**.
  - We can **just tell** you, **you are the one who's always validating the blocks**. We trust you, and then you distribute the blocks to all of us. So we all have the status of our transactions, but he is our **trust provider**. We could do that. It's still a blockchain.
  - Some people would argue, no, it's not a real blockchain because it's not proof of work and so on, but it still follows the rules.
- We could choose some kind of **lottery protocol** by **randomly selecting (validation) nodes**.
  - So we could do some kind of random algorithm.
  - **Intel did this** with a blockchain, and what they did is, they had a chip, and that was a **trusted chip that needed to run on the miners**, and this chip was computing very good random numbers. And then it was something like, the **next miner will be a miner who is following the random number** or so, and that was trusted then just by the hardware, by the chip, itself.
- The big thing that **everybody is now using** almost is this one here, **proof of stake**.
  - And proof of stake is somehow, it **moves trust** from algorithms **somehow to the institutions, but also into an economic game**.
  - So we are no longer just computing in order to find a random number that satisfies a particular condition.
  - What we say is, okay, if we want to run a blockchain (mining node), **everybody of us** should **put 500 euros into the pot** for it. And 500, let's say 1000 euros. That's quite a lot of money for us to use. Also for me. And from then on, **as soon as we detect somebody cheating, we take the money**. So your **stake** is actually a **1000 euros to run a miner**, and if we detect that you are somehow trying to cheat us, we take your 1000 euros. And by that, we somehow **secure the whole thing by an economic game**, more or less.
  - In total, with Ethereum, we see it later on, it's **32 ETH times 3000**. So it's approximately 100,000 euros that you put as a **stake** into the system, and this stake will be taken away if the community detects that your miner is cheating.

## slide: "Grundprinzip von Proof of Stake"

student1: But for mining, I don't know how it works with Ethereum, but for mining one block, **you still have to solve the crypto puzzle, right?**

prof: **No. You're just being assigned to collect the transactions and create the new block. You don't have to solve the crypto puzzle.**
You are just authorized to do it next time.
And therefore, we don't need to solve any crypto puzzle,
and **therefore we don't need to provide energy** for transportation.
And therefore, the **shift** from Ethereum, the proof of work, to proof of stake, **saved, as estimates say, 99% of the energy**.

student2: So you mentioned that each of us is like 1000 euros.
Are we a single node or is each person a single node?

prof: Each person is a single node.

---

### ChatGPT: "in proof of stake how is the validator selected"

In **Proof of Stake (PoS)**, the validator is selected **algorithmically**, usually with a mix of **how much stake they have locked** and some form of **randomization**. Instead of miners racing with electricity like in Proof of Work, the network chooses who gets to propose or validate the next block.

#### Core Idea

If you lock coins as stake, you become eligible to validate blocks.
The more stake you lock, the **higher your probability** of being chosen — but it’s usually not a simple guaranteed turn.

Think of it like a weighted lottery:

* 1 coin staked = 1 ticket
* 100 coins staked = 100 tickets
...
The protocol uses randomness and gives larger stakers better chances.

##### 2. Round-Robin / Committee Selection

Used in some systems.

A group of validators is chosen for a time slot or epoch, and turns are assigned.

##### 3. Reputation / Performance Factors

Some networks also consider:

* uptime
* past honest behavior
* response speed
* missed blocks
...
* validators who stake 32 ETH (or via pools)
* random selection for committees
* one validator proposes a block each slot
* others attest (vote) on it

So it’s not just “largest wallet always wins.”

#### Why Randomness Matters

Without randomness:

* biggest staker could dominate predictably
* easier attacks
* centralization risk

So protocols use cryptographic randomness to keep selection fair.

#### Short Version

In PoS, validators are selected by the protocol using a **stake-weighted random process**, often with extra rules for fairness and reliability. More stake improves odds, but does not guarantee every block.
...
#### 1. Validator deposits stake

To become a validator, someone must lock **32 ETH** in a staking contract.

#### 2. If a validator cheats

Examples of cheating include:

* **Double signing blocks** (attesting to two conflicting versions of the chain)
* **Proposing conflicting blocks**
* **Surround votes** (a specific consensus rule violation)

These are detectable by other validators.

#### 3. The network slashes the validator

When the violation is proven:

* A **portion of the validator’s staked ETH is destroyed (“burned”)**
* The validator is **removed from the validator set**
* They must **exit the network**

#### 4. Who gets the money?

Important point: **Usually nobody gets it.**

Most of the slashed ETH is **burned by the protocol** (removed from circulation).

However:

* The validator who **submits proof of the violation** (a *whistleblower*) may receive a **small reward**.
* The rest is **permanently destroyed**.

#### 5. Why this works

Because validators have **real money locked up**, cheating becomes extremely expensive. If they try to attack the network, they risk losing their stake.

So in short:

* Validator cheats → **proof submitted by another validator**
* Protocol **slashes the stake**
* **Small reward to whistleblower**
* **Most of the ETH is burned**

No single node or authority “takes all the money”—the system punishes the attacker automatically.

---

If you want, I can also explain **why Ethereum burns most of the slashed ETH instead of giving it to other validators** (the reason is actually pretty clever for game theory).
...
“Why not reward the other validators with the attacker’s stake?”

Because that would create **dangerous incentives**.
...
* Validators could coordinate to cause network conditions that increase the chance another validator accidentally violates a rule.
* Then they would **profit from that validator being slashed**.

By **burning most of the stake**, nobody can make big money from someone else’s punishment.
...
* Two validators cooperate.
* One intentionally cheats and gets slashed.
* The other collects the reward.
* They share the profits.

Burning the funds **eliminates this possibility**.
...
* Validators behave honestly because **they risk losing their own money**.
* Other validators report violations only to **protect the network**, not to farm rewards.

So Ethereum gives:

* **A small whistleblower reward**
* **Most of the slashed ETH is burned**
...
* Cheating validator → stake slashed
* Small reward → validator who reports it
* Most ETH → **burned (removed from supply)**

The punishment hurts the attacker **without turning slashing into a profit game for others**.

---

If you're studying blockchain concepts, another interesting thing is **how Ethereum punishes *groups* of attackers much more severely than single attackers**. The penalty scales dramatically. I can explain that mechanism too if you want.
...
Or **we give him a particular access, we allow it to him**. But **we organize that as a community. It's not just for free**.
...
### ChatGPT: difference between "Private blockchain network with permissionless access" and "Private blockchain network with permissioned access"

The difference is about **who can participate** in the blockchain network.

A **private blockchain** means the network itself is controlled by a specific organization or consortium rather than being open like Bitcoin or Ethereum.
Within that private setting, access can still be **permissionless** or **permissioned**.

---

#### 1. Private blockchain network with permissionless access

* The **infrastructure is privately operated**.
* But **anyone can join and interact** without needing approval.
* Users may be free to read data, submit transactions, or run nodes depending on design.
* Governance/control remains with the owner(s), even though participation is open.

##### Example idea:

A company runs a blockchain for digital collectibles. Anyone can create a wallet and use it, but the company controls upgrades and validator rules.

##### Meaning:

Private ownership + open participation.

---

#### 2. Private blockchain network with permissioned access

* The **infrastructure is privately operated**.
* **Only approved participants** can access the network.
* Joining requires invitation, KYC, business relationship, or admin approval.
* Often used by enterprises, banks, supply chains, healthcare.

##### Example idea:

Several banks share a blockchain where only member banks can run nodes and submit transactions.

##### Meaning:

Private ownership + restricted participation.

---

#### Simple Comparison

| Feature               | Private + Permissionless | Private + Permissioned   |
| --------------------- | ------------------------ | ------------------------ |
| Who controls network? | Private entity/group     | Private entity/group     |
| Who can join?         | Anyone                   | Approved members only    |
| Typical use           | Consumer platforms       | Enterprise collaboration |
...
Many people assume **private = permissioned**, because that is the most common combination.
A **private + permissionless** system is possible, but less common.

---

#### One-line summary

* **Private + permissionless** = controlled by few, usable by many.
* **Private + permissioned** = controlled by few, usable only by approved participants.
...
It could be that on a Saturday evening,
we all decide to do some kind of modifications to the blockchain.
To manipulate some data.
And in order to avoid that, we do some kind of anchoring our blockchain.
Let's say **every day at 12 o'clock**, we keep the **hash of our blockchain** and **store that particular hash in Ethereum**.
...
This passage is explaining **why some organizations use a private or faster blockchain that periodically commits proof to a public blockchain** like Ethereum or Bitcoin.

It mixes concepts a bit, so here’s the clean meaning.

---

#### Core Idea

A **public Layer 1 blockchain** (like Ethereum or Bitcoin):

* highly decentralized
* hard to tamper with
* transparent
* secure
* but can be slower or more expensive

So instead of putting every transaction directly on Layer 1, a company or consortium may run a **separate chain/system** that is:
...
Then they periodically **anchor** it to Layer 1.

---

#### What “Anchoring” Means

They take a snapshot of the private chain’s current state and create a **cryptographic hash** of it.

That hash is then written onto Ethereum.

Example:

* At 12:00 every day
* hash of internal blockchain state = `0xABC123...`
* store that hash in Ethereum

Now Ethereum acts like a **public timestamped notary**.

---

#### Why This Helps

Later, if someone changes past records on the private chain:

* the new blockchain state produces a different hash
* it no longer matches the hash stored on Ethereum
* everyone can detect tampering

So even if insiders collude later, they cannot secretly rewrite history **without being exposed**.

---

#### The Saturday Evening Example

The speaker says:

> everyone trusts each other, but not completely

Meaning:

A consortium may behave honestly most of the time, but members could later try to:
...
Because a trusted public chain already stored yesterday’s hash, those edits become detectable.

---

#### Is This Really “Layer 2”?

Not always in the strict technical sense.

True Layer 2s on Ethereum usually inherit security more directly (rollups, state channels, etc.).

What the speaker describes is often closer to:

* sidechain
* consortium chain
* permissioned chain with checkpointing
* anchored private ledger

Not every anchored chain is a formal Layer 2.

---

#### One-Line Summary

They use a fast/private blockchain for daily operations, then periodically store its hash on a public blockchain so nobody can secretly rewrite the past later.

---

### ChatGPT: "what is quorum"

Quorum is an **enterprise-focused blockchain platform** based on Ethereum technology, originally developed by JPMorgan Chase (later open-sourced and then associated with ConsenSys).

It was designed for **private / permissioned blockchain networks** where known organizations transact with each other.

---

#### What Quorum Is

Quorum modifies Ethereum so companies can run a blockchain with features businesses often want:

* only approved participants can join
* faster transactions / quicker finality
* private transactions between selected parties
* controlled governance
* no need for public anonymous miners

So instead of anyone joining like on Ethereum mainnet, Quorum networks usually consist of banks, companies, or institutions.
...
* shared records among companies
* tokenized assets in closed ecosystems
...
Quorum uses Ethereum-style components such as:

* smart contracts
* EVM compatibility
* accounts / transactions

But it swaps the open public model for a permissioned one.

---

#### Is Quorum a Layer 2?

Usually **no**, not in the strict modern sense.

Quorum is more accurately:

* a private Ethereum-based blockchain
* a permissioned consortium chain
* an enterprise ledger platform

It can be **anchored** to a public chain, but that does not automatically make it a true Layer 2 like rollups.

Your professor is using “layer two” in a broader architectural sense.

---

#### Why the Name “Quorum”

A quorum means the minimum number of participants needed to approve decisions. That fits blockchains where a set of known validators reach agreement.

---

#### One-Line Summary

Quorum is a permissioned Ethereum-based blockchain platform for enterprises that want smart contracts and shared ledgers without using the public Ethereum network.
...
### ChatGPT: "explain this picture"

This diagram is a **sequence diagram** showing how Polygon uses Ethereum as a security anchor.

Read it **top to bottom** (time flows downward).

---

#### The Columns

Each vertical lane is one participant:

1. **User** – person/app sending a transaction
2. **Polygon_Validator** – validator that helps confirm Polygon blocks
3. **Polygon_Chain** – the Polygon blockchain itself
4. **Ethereum_L1** – Ethereum Layer 1 mainnet

The dashed vertical lines mean each participant continues existing over time.

---

#### Step-by-Step Flow

##### 1. User sends transaction to Polygon

Arrow:

**User → Polygon_Chain**

This means the user submits a transaction (send tokens, use dApp, etc.) to Polygon instead of directly to Ethereum.

Why? Usually cheaper and faster.

---

##### 2. Polygon validator validates and finalizes block

Arrow between validator and Polygon chain:

**Polygon_Validator ↔ Polygon_Chain**

Validators process pending transactions, create blocks, and finalize them on Polygon.

So the user’s transaction gets included in a Polygon block.

---

##### 3. Submit checkpoint (Merkle Root)

Arrow:

**Polygon side → Ethereum_L1**

After some time, Polygon sends a **checkpoint** to Ethereum.

That checkpoint is usually a **Merkle root** or state commitment:

* one compact hash
* summarizes many Polygon blocks / transactions

Instead of posting every transaction, only the summary is posted.

---

#### What Is a Merkle Root?

A Merkle root is a single hash representing many pieces of data.

If any Polygon transaction changed, the Merkle root would change too.

So it acts like a tamper-evident fingerprint.

---

#### 4. Ethereum stores checkpoint in smart contract

Text on right:

**store Checkpoint in Smart Contract**

Ethereum receives the checkpoint and stores it in a smart contract.

That means the checkpoint becomes:
...
The green text references an address page on Etherscan where such a contract can be viewed.

---

#### 5. Security anchor (can verify state)

Dashed arrow back toward user:

**security anchor (can verify state)**

Meaning:

Later, anyone can compare Polygon data against the checkpoint stored on Ethereum.

If Polygon history were secretly changed afterward, it would no longer match the Ethereum checkpoint.

So Ethereum acts like a trusted notary.

---

#### Big Picture

Polygon does the fast/cheap transaction processing.
Ethereum stores periodic cryptographic proofs of Polygon’s state.

That gives:

* speed from Polygon
* stronger trust anchor from Ethereum

---

#### One-Line Summary

The user transacts on Polygon, Polygon validators finalize blocks, then Polygon periodically posts a Merkle-root checkpoint to Ethereum so Ethereum can serve as a public proof of Polygon’s history.
...
This passage explains **why large data usually stays off-chain** and how a network like Polygon uses Ethereum for additional security by posting summaries/checkpoints.

---

#### 1. Large Files Are Usually Not Stored Directly on Blockchain

The speaker says lecture slides would not be put directly into a blockchain.

Why?

Because blockchains are expensive and inefficient for large files:

* every node would need to store the data
* storing many MB/GB costs a lot
* slower synchronization
* unnecessary for most use cases

So instead:

* keep the real file elsewhere (server, cloud, IPFS, database, etc.)
* compute a **hash** of the file
* store only the hash on-chain

That way you can later prove the file was unchanged.

If one letter changes, the hash changes.

---

#### 2. What Polygon Is Doing Here

The speaker describes a chain like Polygon processing transactions first.

Flow:

1. User sends transaction to Polygon
2. Polygon validators process transactions
3. Polygon creates/finalizes blocks on its own chain
4. Periodically Polygon sends a **checkpoint** to Ethereum

So users don’t need every transaction to happen directly on Ethereum.

---

#### 3. What Is the Checkpoint?

The checkpoint is a compressed proof / summary of Polygon’s state, often involving **Merkle roots**.

A **Merkle root** is a single hash representing many transactions or balances.

So instead of sending thousands of transactions to Ethereum, Polygon can send one compact commitment.

---

#### 4. Why Send It to Ethereum?

Because Ethereum is highly decentralized and hard to tamper with.

Posting checkpoints to Ethereum gives:

* public timestamping
* auditability
* stronger security guarantees
* reference point for dispute resolution / proofs

So Ethereum becomes a trust anchor.

---

#### 5. “Everybody Can Verify the State” Means

Anyone can compare:

* Polygon data
* Merkle proofs
* checkpoint stored on Ethereum

to verify certain transactions or states are consistent with what Polygon committed.

It does **not** mean Ethereum re-executes every Polygon transaction automatically.

It means Ethereum stores a trusted cryptographic commitment.

---

#### 6. Small Correction

Calling Polygon simply “such a chain” is simplified. Polygon has had multiple products/architectures (PoS chain, zkEVM, etc.). The description sounds closest to the **Polygon PoS chain checkpointing to Ethereum**.

---

#### One-Line Summary

Polygon handles transactions cheaply on its own chain, then periodically posts a Merkle-hash checkpoint to Ethereum so the Polygon state can be publicly verified later.
...
And then there should be something like, ... it's using proxies, but ... there must be something like `Submit Checkpoint`. Yeah, that's the method.
...
This passage describes **how two blockchains can be linked through periodic commitments from one chain to another**.

The speaker is simplifying the architecture, but the core idea is valid.

---

#### Plain Meaning

* Polygon processes many transactions on its own chain.
* Every ~20 minutes, a system on Polygon computes a **summary hash/checkpoint** of recent Polygon state.
* That checkpoint is sent to a **smart contract on Ethereum**.
* Ethereum stores that checkpoint permanently.

So Ethereum becomes the public record proving what Polygon’s state looked like at that moment.

---

#### What “Checksum” Means Here

The speaker likely means:
...
Not a simple error-detection checksum like in files/networking.

It’s a compact fingerprint of Polygon data.

If Polygon history were altered later, the fingerprint would no longer match.

---

#### Why Use a Smart Contract on Ethereum?

Instead of manually posting data, a smart contract can:

* receive checkpoints
* store them on-chain
* verify formatting/signatures
* track sequence/order
* be used later for proofs or withdrawals

So the relationship between chains is automated by code.

---

#### “On Polygon We Have Another Smart Contract”

This likely means a contract/system component on Polygon that helps coordinate:

* collecting validator approvals
* packaging checkpoints
* triggering submission to Ethereum

In practice, some of this may be protocol-level infrastructure rather than literally “just one smart contract.”

---

#### Why Every 20 Minutes?

Periodic batching means:

* cheaper than posting every transaction to Ethereum
* faster activity on Polygon
* regular security anchoring

So many Polygon transactions can be represented by one Ethereum submission.

---

#### One-Line Summary

Polygon runs transactions cheaply on its own chain, then regularly sends a cryptographic snapshot of its state to an Ethereum smart contract so Ethereum can serve as a trusted anchor.

---

## slide: Ethereum and L2s (Architecture)

### ChatGPT: "meaning of slide text"

This describes the **basic architecture of a rollup** (a Layer 2 scaling system on top of a Layer 1 like Ethereum).

The key actor here is the **sequencer**.

---

#### What Is a Sequencer?

A sequencer is the component/operator that:

* receives users’ Layer 2 transactions
* orders them into a sequence
* bundles them into batches
* later submits that batch to Layer 1

Think of it as the temporary traffic controller for the rollup.

---

#### Line-by-Line Meaning

##### “A sequencer receives rollup transactions”

Users send transactions to the rollup instead of directly to Ethereum.
...
The sequencer collects these transactions.

---

##### “Determines the order of transactions”

The sequencer decides:

* which transaction comes first
* which comes second
* which go into the next batch

Ordering matters because it affects balances, swaps, liquidations, etc.

---

##### “Gives a pre-confirmation that the transaction will later be contained in an L1 block”

This means the sequencer quickly tells the user:

> Your transaction has been accepted and will be included.

This is **not yet final Ethereum settlement**.

It is an early confirmation so users don’t need to wait for Layer 1 block times.

Later, the batch is actually posted to Ethereum.

---

##### “As a result: lower fees and faster confirmations for users”

Because many transactions are bundled together:

* Layer 1 gas cost is shared across many users
* confirmations feel near-instant
* cheaper than transacting directly on Ethereum

---

#### Example Flow

1. You submit a swap on a rollup
2. Sequencer accepts it in seconds
3. UI shows confirmed/pending success
4. Later sequencer posts batch to Ethereum
5. Final settlement secured by Ethereum

---

#### Important Caveat

The sequencer can improve UX, but it may introduce temporary centralization if only one sequencer exists.

Many rollups aim to decentralize sequencers over time.

---

#### One-Line Summary

A sequencer is the rollup’s transaction organizer: it quickly accepts and orders Layer 2 transactions, then later batches them onto Layer 1, giving users faster and cheaper transactions.

---

### ChatGPT: "what is a rollup"

A **rollup** is a **Layer 2 blockchain scaling system** that executes transactions **outside the main Layer 1 chain** (usually Ethereum), then **bundles (“rolls up”) many transactions into compressed data** and posts that data back to Layer 1.

It is designed to make transactions:

* cheaper
* faster
* more scalable

while still inheriting much of Layer 1’s security.

---

#### Core Idea

Instead of Ethereum processing every single transaction individually:

* 10,000 users transact on the rollup
* the rollup processes them off-chain / on Layer 2
* results are compressed into a batch
* batch data/proofs are sent to Ethereum

So Ethereum secures summaries, not every computation directly.

---

#### Why It’s Called “Rollup”

Because many transactions are **rolled up into one batch** before posting to Layer 1.

Think:

1000 transactions → 1 submission to Ethereum.

---

#### How It Works (Simple Flow)

1. Users send transactions to rollup
2. Sequencer orders and executes them
3. New balances/state are computed
4. Batch is posted to Ethereum
5. Ethereum stores data / verifies proofs
...
Use zero-knowledge proofs to mathematically prove correctness.
...
They help Ethereum scale without sacrificing decentralization as much as putting everything on one faster centralized chain.

---

#### Rollup vs Sidechain

* **Rollup** = relies heavily on Ethereum for security/data availability
* **Sidechain** = separate blockchain with its own security model

---

#### One-Line Summary

A rollup is a Layer 2 system that processes many transactions cheaply off the main chain, then posts compressed results to Ethereum for security.

---

## Energy Consumption of Bitcoin: Does it correlate with the amount of transactions?
...
### Gemini: "would increasing block size reduce the amount of energy consumed by bitcoin"

Increasing the block size does not reduce total network energy consumption, but it does significantly reduce the energy consumed **per transaction**. Because Bitcoin’s Proof-of-Work algorithm requires the same amount of computational energy to mine a block regardless of how full it is, larger blocks allow more data to be processed for the same power cost.

#### How Block Size Affects Efficiency

- **Total Energy**: Bitcoin’s total energy consumption is primarily driven by the network hashrate (the total computational power dedicated to solving blocks), which is influenced by the price of Bitcoin and block rewards, not the block size.
- **Energy per Transaction**: Because the energy cost to produce a block is constant, processing 10,000 transactions in a block instead of 1,000 means the energy footprint is distributed over a much larger volume of activity.
- **Layer 2 Solutions**: Rather than changing the base block size, the Bitcoin network manages energy efficiency at scale using Layer 2 protocols like the Bitcoin Lightning Network. This enables millions of transactions to settle off-chain, drastically lowering the overall energy cost per individual transfer.

#### The Trade-Offs of Larger Blocks

- While larger blocks improve transactional efficiency, the Bitcoin community generally restricts block sizes to prioritize security and decentralization. Larger blocks require more bandwidth and storage, which increases the cost of running a full node and can lead to network centralization.
- To explore the exact electricity used by the network compared to traditional payment processors, you can review the Bitcoin Energy Consumption Index by Digiconomist.

---

### Gemini: "how does block size influence mining difficulty"

In blockchain networks like Bitcoin, block size and mining difficulty are fundamentally independent. **Block size** dictates the amount of transaction data included, while **mining difficulty** represents the mathematical challenge required to successfully validate a block and secure the network.

While block size does not directly alter the underlying cryptographic math required to solve a block, it influences mining difficulty through several indirect mechanisms:

- **The Difficulty Adjustment Algorithm**: Networks aim to produce blocks at a consistent interval (e.g., every 10 minutes for Bitcoin). If the total computing power (hashrate) on the network fluctuates, the difficulty automatically adjusts upward or downward to maintain this pace. Block size does not trigger this adjustment, but the targeted block time acts as the primary control for difficulty.
- **Propagation and Latency**: Larger blocks take more time to propagate across the network to all node validators. If blocks become too large, the time it takes for a newly mined block to reach the majority of miners increases. During this "propagation lag," miners may unknowingly continue working on the previous block. This orphan risk (mining a block that is ultimately rejected by the network) disproportionately affects smaller mining operations.
- **Network Decentralization**: The block size limit restricts throughput (e.g., roughly 7 transactions per second on Bitcoin). Artificially increasing the block size to process more data per block increases the hardware, bandwidth, and storage costs required to run a full node. If running a node becomes too expensive, fewer individuals will participate, leading to centralization. A highly centralized network is vulnerable to security risks, which can impact the consensus rules that enforce difficulty.
- **Transaction Fee Markets**: When block space is limited and demand for transactions is high, users bid on priority inclusion, creating a competitive fee market. Miners collect these transaction fees on top of the base block reward. Since larger blocks accommodate more transactions per block, they impact the volume of fees miners can collect at once, which influences overall mining profitability and the total computational power miners are willing to allocate to the network.

Understanding how these variables operate within the consensus mechanism is key to grasping blockchain scalability. To learn more about the cryptographic mathematics behind these mechanics, you can explore guides like the [Bitcoin Mining Difficulty Explanation](https://learnmeabitcoin.com/technical/mining/target/).

---

### Gemini: "why does increasing the block size to process more data per block increase the hardware, bandwidth, and storage costs required to run a full node"

Increasing the block size exponentially scales the resource requirements for a Full Node by forcing computers to process, transmit, and store significantly more transaction data. This directly limits decentralization by making the hardware too expensive for average users to operate.

#### Why Resource Costs Increase:

- **Storage Costs**: Every valid transaction is stored permanently in the ledger. If a blockchain processes 100 times more data per block, its ledger grows 100 times faster, quickly filling up standard hard drives and requiring expensive, high-capacity Solid State Drives (SSD) to keep up.
- **Bandwidth Costs**: Full nodes do not just download blocks; they must relay them to other network nodes. Larger blocks require significantly more upload and download bandwidth. Operating a node with large block sizes can easily exhaust monthly ISP Data Caps and overwhelm slower internet connections.
- **Hardware / Processing Costs**: Every node must mathematically verify each transaction and signature in a block to ensure network rules are obeyed. Validating large blocks requires a powerful CPU and ample RAM. If block sizes are too high, the time it takes a node to process a block can exceed the time it takes to find the next block, causing the node to fall out of sync with the network.

#### The Centralization Risk

If running a full node requires a costly, enterprise-grade server, regular users are priced out. This forces the network to rely on a smaller number of powerful Data Centers, which compromises the core, Trust-Minimized nature of blockchain technology.

---

### Gemini (answer 1): "would increasing the block size to process more data per block while at the same time increasing block time increase the hardware, bandwidth, and storage costs required to run a full node"

Increasing both the block size (data per block) and the block time (the interval between blocks) has different, often competing effects on full node requirements. Generally, this combination **increases hardware and storage costs** but can actually **optimize or reduce bandwidth costs** compared to just increasing block size alone.

Here is the breakdown of how this configuration affects your node:

#### 1. Storage Costs: Increased

- **Total Data**: Because you are increasing the maximum data processed per block, your node will accumulate blockchain history much faster.
- **State Growth**: A larger block size allows more transactions to be executed, which generally leads to a rapid increase in the "state" (e.g., account balances, smart contract data). This increases the size of the fast NVMe SSD storage required to keep the node running smoothly.

#### 2. Hardware Costs (Compute): Increased

- **Peak Processing**: Processing a massive block requires a spike in CPU and RAM usage to verify all the signatures and execute the smart contracts. You will need a more powerful processor to avoid falling behind the network tip during peak loads.
- **I/O Bottlenecks**: Writing this much data to the disk all at once requires a high-performance SSD.

#### 3. Bandwidth Costs: Decreased (Relative to High Throughput)

- **Propagation Efficiency**: Increasing the block time gives the network more time to gossip and propagate the large blocks to all nodes before the next block arrives.
- **Overhead**: By batching more transactions into a single, larger block spread over a longer time, you reduce the relative overhead of block headers and peer-to-peer messaging. While a single block will require a large data spike to download, the average bandwidth consumption over an hour could remain manageable or even drop compared to a chain with short blocks.

#### Summary

While giving the network more time to process large blocks saves you from needing a gigabit-tier internet connection, the sheer volume of data being processed and stored means your **CPU, RAM, and SSD requirements will be significantly higher** than on a standard blockchain network.

---

### Gemini (answer 2): "would increasing the block size to process more data per block while at the same time increasing block time increase the hardware, bandwidth, and storage costs required to run a full node"

Yes, increasing the amount of data processed per block will almost always increase the hardware, storage, and bandwidth costs required to run a full node.
However, adjusting the block time (the frequency of new blocks) offsets some of these impacts, resulting in nuanced demands on your system.

## How the Adjustments Impact Your Node

* **Storage Costs**: This depends entirely on the throughput (the amount of data processed over a set period, like an hour or a day).
  * *If you increase the block size but proportionally increase the block time* (e.g., blocks are twice as large, but arrive half as often), the net amount of data added to the ledger remains the same. Your long-term storage requirements will grow at the same historical rate.
  * *If you increase the block size without a perfectly proportional increase in block time*, the network processes more data overall. This causes the ledger size to expand faster, requiring you to upgrade to larger Solid State Drives (SSDs) sooner.
* **Bandwidth Costs**: When a new block is mined, your node must download it to verify the data and upload (propagate) it to other connected peers.
  * Larger blocks require a spike in data transfer over a shorter window. This means a higher temporary demand on your internet connection, which can become an issue for node operators on unmetered but slower residential lines.
* **Hardware (CPU/RAM) Costs**: Processing larger blocks places a heavier computational burden on your node's hardware.
  * Validating transactions requires cryptographic signatures and state updates, which spike during block processing. Increasing block size means your node has to perform these calculations in larger batches, requiring faster processors (CPUs) and more RAM to avoid becoming out-of-sync with the rest of the network.

## Why the Balance Matters

Even with longer block times giving your hardware a break between processing spikes, increasing block sizes heavily centralizes the network. If hardware and bandwidth requirements become too expensive to run standard full node software, fewer everyday users will host nodes, which compromises the network's overall decentralization and censorship resistance.

Could you tell me which specific blockchain network or protocol you are analyzing? I can provide you with the exact hardware specifications, disk space requirements, and bandwidth metrics for your setup.
