...
- When a function uses an external variable, it is good programming practice to declare the variable within the function using the `extern` keyword (see [Storage-class specifiers](https://en.cppreference.com/w/c/language/storage_duration))
...
- `setrlimit()`: use `RLIMIT_DATA` [resource limit]({% post_url 2022-09-22-notes-OS %}#resource-limits) to limit the heap size
    -  If you try to extend the data segment past the `RLIMIT_DATA` limit (`brk`, `sbrk` or some intermediary like `malloc`), the attempt will fail. `brk` or `sbrk` will return `< 0` and `malloc` will return a null pointer.
        - but `malloc` uses the `mmap` system call which creates mappings in **virtual memory** and not in `RLIMIT_DATA` !
        - use `ulimit -Sv kiloBytes` to limit the program's virtual memory! (Note: `kiloBytes` must be sufficiently large, i.e. $\approx\gt 3000$ kiloBytes!)
