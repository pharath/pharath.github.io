...
- **the \* character**: value of the next argument of `printf()` (which must be an `int`) is used as field width, e.g. in `printf("%*d", w, a);` the integer `w` specifies the field width
...
Remember:
- **Unicode** defines the **character code**
- **UTF-8** defines the **byte code** for storage
