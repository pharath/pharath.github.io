...
- **Warning:** You can `delete` a destructor. However, it is not possible to define an object or delete a pointer to a dynamically allocated object of a type with a deleted destructor.
...
- example, equivalent to the synthesized `Sales_data` destructor
```cpp
