...
- **roughly**: default initializes class types, zero-initializes all others
...
- copy:
  - **always** synthesized (if we do not declare our own **copy constructor** or **copy-assignment operator**)
- move:
...
- **better:** avoid giving `shared_ptr` responsibility for a built-in pointer
  - ie. if `x` is a built-in pointer, do not use `shared_ptr<int>(x)`
...
- throwing an exception 
  - **terminates** the current function and
...
- if it appears in other places: `terminate` is called
